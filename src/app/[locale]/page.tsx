...
const BRAND_VALUES = [
  {
    title: "Body Positive",
    description:
      "Designed for every curve, every angle, every form. Luxury that celebrates you exactly as you are.",
    icon: Heart,
  },
  {
    title: "Inclusive Sizing",
    description:
      "From 30A to 42H, our pieces are engineered for fit, not compromise. Because luxury has no size limit.",
    icon: Ruler,
  },
  {
    title: "Discreet Luxury",
    description:
      "Unmarked packaging, private billing, and delivery that respects your privacy. Your secret is safe with us.",
    icon: Package,
  },
];
...
export default async function HomePage({
  ...
  return (
    <>
      ...
      >
        {/* Dark overlay */}
        <div className="absolute inset-0 bg-brand-dark/30" aria-hidden="true" />
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <h1 className="font-display text-7xl font-light tracking-[0.3em] text-text-primary md:text-9xl">
            {t.brand}
          </h1>

          <p className="mt-6 font-body text-lg tracking-widest text-text-secondary">
            For the woman who owns her night.
          </p>

          <Link
            href="/collections"
            className="mt-12 inline-block rounded bg-brand-gold px-10 py-4 font-accent text-xs font-medium uppercase tracking-widest text-brand-dark transition-colors hover:bg-brand-gold/90"
          >
            ...
            {collections.map((collection) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  ...
                  <span className="mt-2 inline-flex items-center gap-1 font-accent text-[11px] uppercase tracking-widest text-brand-gold transition-colors group-hover:text-brand-gold/80">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 lg:px-8">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
          {CATEGORIES.map((cat) => (
            ...
            >
              <cat.icon className="h-7 w-7 text-brand-gold transition-transform duration-300 group-hover:scale-110" />
              <span className="font-accent text-xs uppercase tracking-widest text-text-secondary transition-colors group-hover:text-text-primary">
                {cat.label}
              </span>
            </Link>
          ))}
      ...
      <section className="bg-brand-dark">
        <div className="mx-auto flex max-w-7xl flex-col items-center px-6 py-20 text-center lg:px-8">
          ...
          <Link
            href="/collections"
            className="mt-10 inline-block rounded bg-brand-gold px-10 py-4 font-accent text-xs font-medium uppercase tracking-widest text-brand-dark transition-colors hover:bg-brand-gold/90"
          >
            Start Building
          </Link>
        </div>
      ...
      <section className="bg-brand-secondary">
        <div className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
          <h2 className="mb-14 text-center font-display text-3xl font-light tracking-[0.15em] text-text-primary">
          ...
          <div className="grid grid-cols-1 gap-10 md:grid-cols-3">
            {BRAND_VALUES.map((value) => (
              ...
              >
                <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-brand-primary">
                  <value.icon className="h-6 w-6 text-brand-burgundy" />
                </div>
      ...
      <section className="bg-brand-primary py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="mb-10 flex items-center justify-center gap-3">
            <span className="text-text-primary">{INSTAGRAM_ICON}</span>
            <h2 className="font-display text-3xl font-light tracking-[0.1em] text-text-primary">
              Follow @nocturne
            </h2>
          </div>
